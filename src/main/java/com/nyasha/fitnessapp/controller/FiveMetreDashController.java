...
//	}
	...
	@ApiOperation(value = "", response = Iterable.class)
	@PutMapping("/{id}")
	public ResponseEntity<ActionResult> update(@PathVariable(required = false) int id, @RequestBody Update5MDashRequest request) {
		ActionResult result = new ActionResult();
		...
		try {
			...
			Athlete athleteInDb = athleteService.findOne(request.getAthleteId());
			if (athleteInDb == null) {
				result.setMessage("Invalid request.");
				return new ResponseEntity<ActionResult>(result, HttpStatus.BAD_REQUEST);
			}
			...
			if (fiveMetreDashInDB == null) {
				...
				fiveMetreDash.setAthlete(athleteService.findOne(request.getAthleteId()));
				fiveMetreDashService.save(fiveMetreDash);
			}
				...
				fiveMetreDashInDB.setAthlete(athleteService.findOne(request.getAthleteId()));
				fiveMetreDashService.save(fiveMetreDashInDB);
			}
